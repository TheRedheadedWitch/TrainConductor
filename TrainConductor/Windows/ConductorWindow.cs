...
internal class ConductorWindow : Window, IDisposable
{
    ...
    private bool showAddMobPopup = false;
    ...
    private readonly TimeSpan _sendCooldown = TimeSpan.FromSeconds(3);
    ...
    public override unsafe void Draw()
    {
        ...
        ImGui.BeginChild("mob_list_panel", new Vector2(-1, colAvail.Y - 50), true);
        ...
        if (ImGui.BeginTable("MobTable", 2, ImGuiTableFlags.RowBg | ImGuiTableFlags.ScrollY))
        {
            ...
            for (int i = 0; i < displayList.Count; i++)
            {
                ...
                if (ImGui.BeginTable($"inner_{i}", 4, ImGuiTableFlags.SizingStretchProp))
                {
                    ...
                    if (ImGui.BeginDragDropTarget())
                    {
                        ...
                        if (payload.Data != null)
                        {
                            ...
                            if (sourceIdx != i && sourceIdx >= 0 && sourceIdx < displayList.Count)
                            { moveSrc = sourceIdx; moveDst = i; }
                        }
                ...
                if (!upEnabled) ImGui.PushStyleVar(ImGuiStyleVar.Alpha, 0.5f);
                if (ImGui.Button($"▲##up{i}") && upEnabled) MoveMobByDirection(displayList[i], -1);
                if (!upEnabled) ImGui.PopStyleVar();
                ImGui.SameLine();
                if (!downEnabled) ImGui.PushStyleVar(ImGuiStyleVar.Alpha, 0.5f);
                if (ImGui.Button($"▼##down{i}") && downEnabled) MoveMobByDirection(displayList[i], 1);
        ...
        ImGui.BeginChild("left_bottom_buttons", new Vector2(-1, 50), false);
        ...
        { storedData.Mobs.Clear(); storedData.Save(); }
        ImGui.SameLine();
        if (ImGui.Button("Add Mob", new Vector2(bottomBtnW, 36f))) showAddMobPopup = true;
        ImGui.EndChild();
        ImGui.EndChild();

        // RIGHT PANEL
        ImGui.NextColumn();
        ImGui.BeginChild("right_panel", new Vector2(-1, -1), false);
        ImGui.Text("Preview");
        string preview = string.Empty;
        if (displayList.Count > 0 && !string.IsNullOrWhiteSpace(storedData.ChatMessage))
            try { preview = ParseData.Parse(storedData.ChatMessage, displayList[0].Instance, 0, 0, displayList[0]); } catch { }
        else if (displayList.Count == 0 && !string.IsNullOrWhiteSpace(storedData.SelectedPatch))
            try { preview = ParseData.Parse(storedData.ChatMessage, 0, 0, 0, new MobLocation()); } catch { }

        ImGui.BeginChild("preview_box", new Vector2(-1, 80), true);
        ...
        if (changed)
        {
            storedData.Yell = yell; storedData.Shout = shout; storedData.Echo = echo; storedData.Party = party; storedData.Say = say;
            storedData.Save();
        }

        // SEND BUTTON WITH COOLDOWN
        TimeSpan timeSinceLastSend = DateTime.Now - _lastSendTime;
        bool canSend = !string.IsNullOrWhiteSpace(storedData.ChatMessage) && timeSinceLastSend >= _sendCooldown;
        string sendText = canSend ? "Send" : $"Send ({Math.Ceiling((_sendCooldown - timeSinceLastSend).TotalSeconds)}s)";
    ...
    private void DrawAddMobPopup()
    {
        if (!showAddMobPopup) return;

        ImGui.OpenPopup("Add New Mob");
        Vector2 center = ImGui.GetMainViewport().GetCenter();
        ImGui.SetNextWindowPos(center, ImGuiCond.Appearing, new Vector2(0.5f, 0.5f));

        if (ImGui.BeginPopupModal("Add New Mob", ref showAddMobPopup, ImGuiWindowFlags.AlwaysAutoResize))
        {
            ...
            if (ImGui.Button("Add", new Vector2(120, 0)) && float.TryParse(newX, out float x) && float.TryParse(newY, out float y) && !string.IsNullOrWhiteSpace(newMob) && !string.IsNullOrWhiteSpace(newZone))
            {
                ...
                newMob = newZone = newX = newY = newInstance = string.Empty;
                storedData.Save();
                showAddMobPopup = false;
            }
            ImGui.SameLine();
    ...
    private unsafe void SendChat()
    {
        ...
        foreach ((bool enabled, string prefix, XivChatType chatType) in chatTargets)
        {
            ...
            {
                Utf8String* utf8String = Utf8String.FromSequence(ptr);
