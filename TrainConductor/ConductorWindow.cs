...
internal class ConductorWindow : Window, IDisposable
{
    ...
    private static void MoveMobBetweenPersistentAndFiltered(MobLocation mobToMove, int newPersistentIndex)
    {
        int currentPersistentIndex = storedData.Mobs.IndexOf(mobToMove);
        if (currentPersistentIndex < 0 || newPersistentIndex < 0 || newPersistentIndex > storedData.Mobs.Count) return;
        storedData.Mobs.RemoveAt(currentPersistentIndex);
        storedData.Mobs.Insert(newPersistentIndex, mobToMove);
        storedData.Save(); _currentCachedPatch = string.Empty;
    }
    ...
    private unsafe void SendChat()
    {
        ...
        foreach ((bool enabled, string prefix) in chatTargets)
            if (enabled)
            {
                SeString finalMessage = MapLinks.ReplaceMapPlaceholder(prefix + msg, targetMob.Zone, targetMob.X, targetMob.Y, instanceId);
